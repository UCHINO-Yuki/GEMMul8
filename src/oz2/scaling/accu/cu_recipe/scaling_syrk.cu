...
#endif
inline constexpr cublasFillMode_t UPLO_C = GEMMUL8_INST_FILLMODE;

} // namespace

static_assert(UPLO_C == CUBLAS_FILL_MODE_UPPER || UPLO_C == CUBLAS_FILL_MODE_LOWER,
              "UPLO_C must be CUBLAS_FILL_MODE_UPPER or CUBLAS_FILL_MODE_LOWER.");

#define INSTANTIATE_THIS(NM)                                         \
    template void scaling_syrk<T, BE, NM, UPLO_C>(                   \
        const cudaStream_t,                                          \
