        ...
        for (size_t n = 4096; n <= 4351; ++n)
            for (const auto ld : ld_cases) {
                ok &= trtrmm_debug::run_case<TA, TB, TC, backend>(
                    ctx, progress, "test_trtrmm_padding",
                    debug_common::Dim3{0, n, 0}, CUBLAS_FILL_MODE_UPPER, CUBLAS_FILL_MODE_UPPER,
                    CUBLAS_OP_N, CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT, CUBLAS_DIAG_NON_UNIT,
                    ld, scal, num_moduli);
            }
        return ok;
    }
