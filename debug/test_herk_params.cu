        ...
        for (auto trans : debug_common::her_op_list<TC>())
            for (const auto dim : dims)
                for (const auto ld : ld_cases) {
                    if constexpr (rankk_debug::is_hermitian_func<gemmul8::Func::herk>()) {
            ...
            for (auto trans : debug_common::her_op_list<TC>())
                for (const auto dim : dims)
                    for (const auto ld : ld_cases) {
                        if constexpr (rankk_debug::is_hermitian_func<gemmul8::Func::herk>()) {
                            for (size_t scalar_idx = 0; scalar_idx < debug_common::real_scalar_cases<TC>().size(); ++scalar_idx)
                                ok &= rankk_debug::run_case_single<gemmul8::Func::herk, TA, TC, backend>(
                                    ctx, progress, "test_herk_params", dim, uplo, trans, ld, num_moduli, true, scalar_idx);
                        } else {
                            for (size_t scalar_idx = 0; scalar_idx < debug_common::scalar_cases<TC>().size(); ++scalar_idx)
                                ok &= rankk_debug::run_case_single<gemmul8::Func::herk, TA, TC, backend>(
                                    ctx, progress, "test_herk_params", dim, uplo, trans, ld, num_moduli, true, scalar_idx);
...
int main() {
    CountRunner counter;
    debug_common::run_all_complex_single_input_type_backend_cases(counter);
    bool ok = true;
    {
        debug_common::Context ctx;
        debug_common::Progress progress(counter.count);
        progress.print();
        ok = debug_common::run_all_complex_single_input_type_backend_cases(Runner{ctx, progress});
        progress.finish();
    }
