...
struct Runner {
    ...
    bool operator()() {
        ...
        for (size_t n = 4096; n <= 4351; ++n)
            for (const auto ld : ld_cases) {
                ok &= rankk_debug::run_case_single<
                    gemmul8::Func::herk, TA, TC, backend>(
                    ctx, progress, "test_herk_padding",
...
int main() {
    CountRunner counter;
    debug_common::run_representative_complex_single_input_type_backend_cases(counter);
    bool ok = true;
    {
        debug_common::Context ctx;
        debug_common::Progress progress(counter.count);
        progress.print();
        ok = debug_common::run_representative_complex_single_input_type_backend_cases(Runner{ctx, progress});
        progress.finish();
    }
