...
struct Runner {
    ...
    bool operator()() {
        ...
        for (size_t n = 4096; n <= 4351; ++n)
            for (const auto ld : ld_cases) {
                ok &= rankk_debug::run_case_double_input<gemmul8::Func::syrkx, TA, TB, TC, backend>(
                    ctx, progress, "test_syrkx_padding",
                    debug_common::Dim3{0, n, n}, CUBLAS_FILL_MODE_UPPER, CUBLAS_OP_N,
                    ld, debug_common::default_scalar_case<TC>(), num_moduli);
            }
...
int main() {
    CountRunner counter;
    debug_common::run_representative_type_backend_cases(counter);
    bool ok = true;
    {
        debug_common::Context ctx;
        debug_common::Progress progress(counter.count);
        progress.print();
        ok = debug_common::run_representative_type_backend_cases(Runner{ctx, progress});
        progress.finish();
    }
