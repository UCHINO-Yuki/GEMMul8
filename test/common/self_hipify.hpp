...
#include "../../src/oz2/common/self_hipify.hpp"
...
namespace getWatt {
inline const char *nvmlErrorString(amdsmi_status_t result) {
    const char *error_string = nullptr;
    amdsmi_status_code_to_string(result, &error_string);
    return error_string;
}

inline amdsmi_status_t nvmlDeviceGetHandleByIndex_v2(unsigned gpu_id, amdsmi_processor_handle *device) {
...
inline amdsmi_status_t nvmlDeviceGetPowerUsage(amdsmi_processor_handle device, unsigned *mw) {
