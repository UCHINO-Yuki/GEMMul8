...
void check_accuracy(
    ...
    const size_t lwork_gemmul8_i8 =
        (run_oz2_i8)
            ? gemmul8::workSize<testTraits<T>::is_complex, gemmul8::Backend::INT8, func>(m_max, n_max, k_max, num_moduli_max)
            : 0;

    const size_t lwork_gemmul8_f8 =
        (run_oz2_f8)
            ? gemmul8::workSize<testTraits<T>::is_complex, gemmul8::Backend::FP8, func>(m_max, n_max, k_max, num_moduli_max)
            : 0;

    const size_t lwork_ozaki1 =
        (run_oz1_i8)
            ? ozaki1::workSize(m_max, n_max, k_max, 1, testTraits<T>::is_complex, 8 * num_slice_max - 1)
            : 0;
...
template void check_accuracy<float>(
    std::string &, std::string &, cublasFillMode_t, cublasSideMode_t,
