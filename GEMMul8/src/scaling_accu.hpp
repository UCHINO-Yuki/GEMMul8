...
__global__ void compute_sft_scaling_rowwise_kernel(
...
__global__ void compute_sft_scaling_colwise_kernel(
    const size_t m,                  // size(C32i,1)
    const int *const C32i,           // input (ldc32i * n)
    const size_t ldc32i,             // leading dimension
    int16_t *const __restrict__ sft, // exponent of shift values
...
__inline__ void scaling(
    ...
    const size_t ldb,             // leading dimension
    int8_t *const A8i,            // output (lda8i * m)
    int8_t *const A8i_high,       // work/output (lda8i * m)
    const size_t lda8i,           // leading dimension
    const size_t incA8i,          // increment between the A8i
    int16_t *const sftA,          // exponent of shift values for rows of A
    int8_t *const B8i,            // output (ldb8i * n)
    int8_t *const B8i_high,       // work/output (lda8i * m)
    const size_t ldb8i,           // leading dimension
    const size_t incB8i,          // increment between the B8i
    int16_t *const sftB,          // exponent of shift values for cols of B
    int32_t *const C32i,          // tmp (ldc32i * n)
    ...
    extract_8i_launch<T>(op_A, op_B, m, n, k, A, lda, B, ldb, A8i_high, lda8i, sftA, B8i_high, ldb8i, sftB, skip_scalA, skip_scalB);

    // C32i := A8i^T*B8i
    constexpr int32_t alpha = 1;
    constexpr int32_t beta  = 0;
    cudaDeviceSynchronize();
    cublasGemmEx(handle, CUBLAS_OP_T, CUBLAS_OP_N, ldc32i, n, lda8i, &alpha, A8i_high, CUDA_R_8I, lda8i, B8i_high, CUDA_R_8I, ldb8i, &beta, C32i, CUDA_R_32I, ldc32i, CUBLAS_COMPUTE_32I, CUBLAS_GEMM_DEFAULT);
    ...
    if (num_moduli <= threshold<T>::x) {
        scaling_launch<T, 1>(op_A, op_B, m, n, k, num_moduli, A, lda, B, ldb, A8i, lda8i, incA8i, sftA, B8i, ldb8i, incB8i, sftB, C32i, ldc32i, log2M, skip_scalA, skip_scalB);
    } else if (num_moduli <= threshold<T>::y) {
        scaling_launch<T, 2>(op_A, op_B, m, n, k, num_moduli, A, lda, B, ldb, A8i, lda8i, incA8i, sftA, B8i, ldb8i, incB8i, sftB, C32i, ldc32i, log2M, skip_scalA, skip_scalB);
    } else if (num_moduli <= threshold<T>::z) {
        scaling_launch<T, 3>(op_A, op_B, m, n, k, num_moduli, A, lda, B, ldb, A8i, lda8i, incA8i, sftA, B8i, ldb8i, incB8i, sftB, C32i, ldc32i, log2M, skip_scalA, skip_scalB);
    } else {
        scaling_launch<T, 4>(op_A, op_B, m, n, k, num_moduli, A, lda, B, ldb, A8i, lda8i, incA8i, sftA, B8i, ldb8i, incB8i, sftB, C32i, ldc32i, log2M, skip_scalA, skip_scalB);
    }
}
